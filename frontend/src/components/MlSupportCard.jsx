...
export default function MlSupportCard({ score }) {
  ...
  return (
    <div className="mt-3 border border-soc-border/50 rounded-md px-3 py-2.5 bg-[#080d14]">
      <div className="text-[10px] uppercase tracking-[0.12em] font-mono text-soc-muted">Supporting ML signal</div>
      <div className="text-xs text-soc-muted mt-0.5">Synthetic/demo model support</div>
      <div className="text-xs font-mono text-slate-500 mt-1 tabular-nums">p(malicious): {p.toFixed(2)}</div>
      <p className="text-[11px] text-soc-muted mt-1.5 leading-snug">
